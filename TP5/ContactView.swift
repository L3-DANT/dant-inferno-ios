...
class ContactView: UITableViewController {
    ...
    @IBAction func url(_ sender: AnyObject) {
        ...
        print("--------------------Début-------------------------------")
        let result = AppelHttp(urldonne: "http://178.62.22.140:8080/api/contact/liste/amis?idUser=1", create: "tabContact") //le parametre create permet de déterminé ce qui sera fait avec                     le résultat de la requete
        print("--------------------Fin---------------------------------")
    
        
       print("got 22222222222222222222222222222222 result: \(result)")
        /*
         
         do {
            let json = try JSONSerialization.JSONObjectWithData(result, options: .AllowFragments)
